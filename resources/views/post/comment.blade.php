...
<div class="mt-5 mb-4 space-y-2">
    <div class="flex space-x-4 items-center justify-center">
        <x-input-label for="comment-update" class="font-semibold text-lg truncate my-3 w-3/12 md:w-1/12"
            :value="'사용자명1111'" />
        <x-text-input id="comment-update" class="w-9/12 md:w-11/12" type="text" name="comment-update"
        ...
        {{-- TODO 모바일화면일시 현재부분부터 2행째로 분리하기 --}}
        <x-text-input id="password-update" type="password" name="password-update" :value="old('password-update')"
            placeholder="{{ __('Password') }}" autocomplete="current-password" autofocus />
        <x-input-error :messages="$errors->get('current-password')" class="mt-2" />
        <svg fill="#58D68D" class="w-7 h-7 cursor-pointer" viewBox="0 -8 72 72">
            <path
        ...
        <svg class="w-6 h-6 cursor-pointer" viewBox="-3 0 32 32" fill="none">
